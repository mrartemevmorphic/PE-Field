...
def apply_rotary_emb_headwise(
    x: torch.Tensor,
    freqs_cis: Tuple[torch.Tensor, torch.Tensor],
    # num_heads: int,
) -> torch.Tensor:
    # x.shape torch.Size([1, 24, 1056, 128])
    cos, sin = freqs_cis  # [S, H*D]
    S = cos.shape[0]
    # HD = cos.shape[1]
    D = x.shape[-1]
    H = x.shape[1]
    ...
    # permute
    cos = cos.permute(1, 0, 2)  # [H, S, D]
    sin = sin.permute(1, 0, 2)

    # add batch dim
    cos = cos[None, :, :, :]  # [1, H, S, D]
    sin = sin[None, :, :, :]  # [1, H, S, D]

    # 后面保留原有逻辑 reshape + rotate
