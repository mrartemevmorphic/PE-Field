...
class FluxAttnProcessor2_0:
    ...
    def __call__(
        ...
        if image_rotary_emb is not None:
            from .embeddings import apply_rotary_emb, apply_rotary_emb_headwise

            if isinstance(image_rotary_emb, list):
                if len(image_rotary_emb) == 3:

                    total_len = query.shape[2]
                    half_n = (total_len - 512) // 2
                    ...
                    if image_rotary_emb[0][0].shape[0] == 512:
                        total_len = query.shape[2]
                        first_part_len = 512
                        second_part_len = total_len - 512
                    else:
                        
                        total_len = query.shape[2]
                        half_n = (total_len - 512) // 2

                        first_part_len = 512 + half_n
                        second_part_len = half_n
                        first_part_len = image_rotary_emb[0][0].shape[0]
                        second_part_len = image_rotary_emb[1][0].shape[0]
                    # Split on dim=2

                    query_first, query_second = torch.split(query, [first_part_len, second_part_len], dim=2)
                    key_first, key_second = torch.split(key, [first_part_len, second_part_len], dim=2)
                    ...
                    # Concatenate back
                    query = torch.cat([query_first, query_second], dim=2)
                    key = torch.cat([key_first, key_second], dim=2)
